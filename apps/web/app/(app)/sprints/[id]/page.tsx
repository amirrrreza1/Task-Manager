...
export default function SprintDetailPage() {
  ...
  async function createCarryOverSprint() {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : 'Could not create the planned sprint.');
    } finally {
  ...
  return <div className="page-stack sprint-detail">
    ...
    <section className="sprint-content">
      <div className="sprint-panel"><h2>{sprint.status === 'COMPLETED' ? 'Final outcome' : 'Sprint work'}</h2>
        {(sprint.status === 'COMPLETED' ? sprint.taskSnapshots : sprint.tasks).map((task) => {
          ...
          return <div className="sprint-work-item" key={task.id}><div className="sprint-task"><span className={completed ? 'done-marker' : 'open-marker'} aria-label={completed ? 'Completed' : 'Not completed'} />
            ...
            <span>{task.estimateValue ? `${task.estimateValue} ${task.estimateUnit === 'HOURS' ? 'h' : 'pts'}` : '—'}</span></div>
      ...
      {sprint.subtasks.length ? <div className="sprint-panel standalone-subtasks"><h2>Planned subtasks</h2><p className="muted">These are planned independently of their parent task.</p>{sprint.subtasks.map((subtask) => <div className="sprint-task" key={subtask.id}><span className={subtask.isCompleted ? 'done-marker' : 'open-marker'} /><div><strong>{subtask.title}</strong><small>From <Link href={`/tasks/${subtask.task.id}`}>{subtask.task.title}</Link>{subtask.assignee ? ` · ${subtask.assignee.displayName}` : ''}</small></div><span>{subtask.estimateValue ? `${subtask.estimateValue} ${subtask.estimateUnit === 'HOURS' ? 'h' : 'pts'}` : '—'}</span></div>)}</div> : null}
      <div className="sprint-panel"><h2>Notes</h2><form className="comment-form" onSubmit={saveComment}><Textarea aria-label="Sprint comment" maxLength={10000} onChange={(event) => setComment(event.target.value)} placeholder="Add a retrospective note or update…" required rows={3} value={comment} /><div><Button variant="primary" size="sm" disabled={busy} type="submit">{editingComment ? 'Save edit' : 'Add comment'}</Button>{editingComment ? <Button variant="ghost" size="sm" onClick={() => { setEditingComment(null); setComment(''); }} type="button">Cancel</Button> : null}</div></form>
